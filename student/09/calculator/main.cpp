...
int main() {
    ...
    while ( true ) {
        ...
        while (i < command_to_be_executed.length()) {
            fixed_command += toupper(command_to_be_executed[i]);
            i += 1;
        }
        ...
        for (auto& c : COMMANDS) {
            if (fixed_command == c.str) {
                ...
                if (c.parameter_number == pieces.size()-1) {
                    ...
                    if (string_to_double(pieces[1], result1) and
                            string_to_double(pieces[2], result2)) {
